...
receivedLen = 500;
bits = [30];
trainInput = zeros(receivedLen * 2, nTrain * size(trainModIndex, 2) * size(trainSNR,2));
trainTarget = zeros(6, nTrain * size(trainModIndex, 2));
valInput = zeros( receivedLen * 2, nVal * size(valModIndex, 2) * size(valSNR,2));
	...
	for j = 1:1:size(valSNR,2)
		for i=1:1:nVal
		    ...
		    for k = 1:1:size(valModIndex,2)
                ...
                received = transpose([(received / max(received))  (tmp123 / max(received))]);
...
for i = 1:1:size(outputs,2)
    outputs(i) = floor(outputs(i) + 0.5);
end
accuracy = 1 - (sum(abs(outputs - valTarget)) / size(outputs,2))
% figure();
% hold on;
